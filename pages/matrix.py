...
def show():
    ...
    <div class="section-header-box">
        <h2>■ 誤差率帯別・ABC区分別マトリクス（AI予測値 vs 計画値）</h2>
        <p>このセクションでは、商品コードごとの誤差率を6つの誤差帯（0～10%、10～20% … 100%以上）に分類し、ABC区分別にマトリクス形式で集計・可視化します。誤差率のタイプは「絶対誤差率」「正の誤差率」「負の誤差率」から選択でき、誤差傾向の把握や欠品・過剰在庫リスクの分析に活用できます。</p>
    </div>
    """, unsafe_allow_html=True)
