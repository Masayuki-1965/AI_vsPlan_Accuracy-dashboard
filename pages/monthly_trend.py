...
def show():
    ...
    <div class="section-header-box">
        <h2>■ 月次推移比較</h2>
        <p>このセクションでは、AI予測値、計画値（複数可）の<strong>月平均絶対誤差率</strong>（詳細は注釈参照）の差分（何ポイント以上か？）を指定し、該当する商品コードを抽出します。その上で、AI予測値・計画値・実績値の月次推移を折れ線グラフで重ねて表示します。これにより「AI予測値の精度はどの程度か？」「誤差の傾向や特徴は？」「どこに改善の余地があるのか？」を把握できます。</p>
    </div>
    """, unsafe_allow_html=True)
