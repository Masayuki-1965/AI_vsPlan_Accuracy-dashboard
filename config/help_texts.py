...
PAGE_DESCRIPTIONS = {
    'upload': {
        'title': "データセット作成",
        'description': "このセクションでは、AI予測値、計画値（複数可）、実績値の月次データ（CSV）を読み込み、分析用のデータセットを作成します。"
    },
    'matrix': {
        'title': "誤差率マトリクス分析（誤差率帯×ABC区分）",
        'description': "このセクションでは、AI予測値、計画値（複数可）の誤差率を6段階の誤差率帯（0～10%、10～20% … 100%以上）に分類し、ABC区分別にマトリクス形式で集計・可視化します。誤差率タイプは「絶対誤差率」「正の誤差率」「負の誤差率」から選択でき、誤差傾向の把握や欠品・過剰在庫リスクの分析に活用できます。"
    },
    'scatter': {
        'title': "誤差率散布図分析（誤差率散布図・予測値vs実績値散布図）",
        'description': "このセクションでは、AI予測値、計画値（複数可）の誤差率を2種類の散布図（「誤差率散布図」「予測値 vs 実績値散布図」）でABC区分ごとに可視化し、各区分における誤差の傾向や特徴を明らかにします。"
    },
    'monthly_trend': {
        'title': "月次推移比較",
        'description': "このセクションでは、AI予測値、計画値（複数可）の誤差率のポイント差を指定して該当商品コードを抽出し、AI予測値・計画値・実績値の月次推移を折れ線グラフで重ねて表示します。「AI予測値の精度はどれくらいか」「改善余地があるとすれば、誤差の傾向や特徴はどこにあるか？」を把握できます。"
    }
}
